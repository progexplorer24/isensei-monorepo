...
function toJson(rules) {
  return Belt_Array.reduceU(rules, {}, ruleToDict);
}
...
function Make(CssImpl) {
  ...
  var merge = function (styles) {
    return CssImpl.mergeStyles(styles);
  };
  ...
  var framesToDict = function (frames) {
    return Belt_Array.reduceU(frames, {}, (function (dict, param) {
                  dict[param[0].toString() + "%"] = toJson(param[1]);
                  return dict;
                }));
  };
  var keyframes = function (frames) {
    return CssImpl.makeKeyframes(framesToDict(frames));
  };
  var renderKeyframes = function (renderer, frames) {
    return CssImpl.renderKeyframes(renderer, framesToDict(frames));
  };
  return {
...
function backdropFilter(x) {
  return {
          TAG: /* D */0,
          _0: "backdropFilter",
          _1: join(Belt_Array.map(x, Css_AtomicTypes$IsenseiMonorepo.BackdropFilter.toString), ", ")
        };
}
...
function borderRadius4(topLeft, topRight, bottomLeft, bottomRight) {
  return {
          TAG: /* D */0,
          _0: "borderRadius",
          _1: Css_AtomicTypes$IsenseiMonorepo.Length.toString(topLeft) + (" " + (Css_AtomicTypes$IsenseiMonorepo.Length.toString(topRight) + (" " + (Css_AtomicTypes$IsenseiMonorepo.Length.toString(bottomLeft) + (" " + Css_AtomicTypes$IsenseiMonorepo.Length.toString(bottomRight))))))
        };
}
...
function fontFamilies(xs) {
  return {
          TAG: /* D */0,
          _0: "fontFamily",
          _1: join(Belt_Array.map(xs, Css_AtomicTypes$IsenseiMonorepo.FontFamilyName.toString), ", ")
        };
}
...
function transforms(x) {
  return {
          TAG: /* D */0,
          _0: "transform",
          _1: join(Belt_Array.map(x, Css_AtomicTypes$IsenseiMonorepo.Transform.toString), " ")
        };
}
...
function firstLetter(__x) {
  return selector("::first-letter", __x);
}

function firstLine(__x) {
  return selector("::first-line", __x);
}

function selection(__x) {
  return selector("::selection", __x);
}

function child(x, __x) {
  return selector(" > " + x, __x);
}

function children(__x) {
  return selector(" > *", __x);
}

function directSibling(__x) {
  return selector(" + ", __x);
}

function placeholder(__x) {
  return selector("::placeholder", __x);
}

function siblings(__x) {
  return selector(" ~ ", __x);
}

function anyLink(__x) {
  return selector(":any-link", __x);
}
...
function fontFace(fontFamily, src, fontStyle, fontWeight, fontDisplay, param) {
  ...
  var src$1 = join(Belt_Array.map(src, (function (x) {
              if (x.NAME === "url") {
                return "url(\"" + x.VAL + "\")";
              } else {
                return "local(\"" + x.VAL + "\")";
...
function strokeDasharray(x) {
  return {
          TAG: /* D */0,
          _0: "strokeDasharray",
          _1: typeof x === "object" ? join(Belt_Array.map(x.VAL, string_of_dasharray), " ") : "none"
        };
}
